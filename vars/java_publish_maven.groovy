...
def call(BuildArgsModel buildArgs) {
    ...
    pipeline {
        ...
        // 安装配置工具
        tools {
            // 安装配置为“args.BUILD_MAVEN_TOOL”的maven版本并将其添加到路径中
            maven buildArgs.buildMavenTool
            // 安装配置为“args.BUILD_JDK_TOOL”的Jdk版本并将其添加到路径中
        ...
        stages {
            ...
            stage('制作产物') {
                ...
                steps {
                    archiveArtifacts artifacts: "${buildArgs.projectName}/${buildArgs.targetPath}", fingerprint: true, followSymlinks: false, onlyIfSuccessful: true
                }
            }
        }
        post {
