...
| CLEAN_CACHE_VALUE         | 清理缓存的默认方式   | 否   | `String`  | no_clean        | 清理缓存控制默认方式，可选：`no_clean` `clean_build` `clean_workspace` `clean_all`<br/>`no_clean`：不清理缓存和构建资源<br/>`clean_build`：清理构建资源，如：`Java`清理`Maven`本地仓库<br/>`clean_workspace`:删除当前工作空间所有内容<br/>`clean_all`：清理包括`clean_build` `clean_workspace` |
| CLEAN_CACHE_PATH          | 清理路径             | 否   | `String`  | 无              | 默认清理默认清理`Maven`整个私仓整个构建路径，`maven`构建并`CLEAN_CACHE_VALUE=clean_build或clean_all`生效<br/>相对于私仓的相对路径，如：`maven`的本地仓库为`/data/maven/repo`，清理路径为`/com/siweite`，实际清理路径为`/data/maven/repo/com/siweite` |
...
| BUILD_COMMAND             | 编译打包命令         | 是   | `String`  | `mvn clean package` | 多行命令使用 `\n` 分割 或 使用 `&&` 或使用 三引号            |
...
| DEPLOY_ENV                | 部署环境             | 否   | `String`  | 无              | 如：`dev` `prod`写入环境变量，可以影响打包、启动读取的环境配置<br/>不配置，则不会添加环境变量 |
| DEPLOY_PORT               | 部署端口             | 否   | `String`  | 无              | 对外可访问端口，非docker内部端口，Docker对外映射端口<br/>可以不用配置，在`Jenkins`参数构建时输入<br/>当`RUN_NETWORK`=`bridge`，此参数必填 |
| RUN_PORT                  | 运行端口             | 否   | `String`  | 8080              | 默认8080端口，容器内部运行端口<br/>当`RUN_NETWORK`=`bridge`，此参数必填 |
